...
/* MODULES */
// import Parse from 'parse/dist/parse.min.js' (removed)
...
async function getDiaryToEdit(param) {
    ...
    try {
        const response = await axios.get(`/api/diaries/${param}`, {
            headers: AuthService.getAuthHeaders()
        });
        
        if (response.data) {
            diary = response.data; // No need to JSON.parse since axios already parses JSON
        ...
        console.error(`Error fetching diary to edit: ${error.message}`);
        console.log(' -> No diary to edit or error occurred')
    }
}
