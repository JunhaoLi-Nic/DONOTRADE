...
export const deviceTimeZone = ref(Intl.DateTimeFormat().resolvedOptions().timeZone) // Store local device timezone
...
// Utility function to ensure dates are at least a day apart
export function ensureDateDifference(startDate, endDate) {
  if (startDate === endDate) {
    return {
      start: startDate,
      end: dayjs.unix(startDate).add(1, 'day').endOf('day').unix()
    }
  }
...
export const selectedTimeFrame = typeof localStorage !== 'undefined' ? ref(localStorage.getItem('selectedTimeFrame') || 'day') : ref('day')
export const selectedRatio = typeof localStorage !== 'undefined' ? ref(localStorage.getItem('selectedRatio') || 'winRate') : ref('winRate')
...
export const selectedGrossNet = typeof localStorage !== 'undefined' ? ref(localStorage.getItem('selectedGrossNet') || 'gross') : ref('gross')
export const selectedPlSatisfaction = typeof localStorage !== 'undefined' ? ref(localStorage.getItem('selectedPlSatisfaction') || 'pl') : ref('pl')
export const selectedBroker = typeof localStorage !== 'undefined' ? ref(localStorage.getItem('selectedBroker')) : ref()
export const selectedDateRange = typeof localStorage !== 'undefined' ? 
  ref(localStorage.getItem('selectedDateRange') ? 
    (() => {
      // Parse stored value
      const storedValue = JSON.parse(localStorage.getItem('selectedDateRange'));
      // Validate timestamps - ensure they're not in the future
      const currentTime = Math.floor(Date.now()/1000);
      // If timestamp is more than a day in the future, use current time
      if (storedValue.start > currentTime + 86400 || storedValue.end > currentTime + 86400) {
        console.warn("Invalid future timestamps detected in selectedDateRange, resetting to current day");
        return {
          start: Math.floor(Date.now()/1000 - 86400),  // yesterday
...
export const selectedMonth = typeof localStorage !== 'undefined' ? 
  ref(localStorage.getItem('selectedMonth') ? 
    (() => {
      // Parse stored value
      const storedValue = JSON.parse(localStorage.getItem('selectedMonth'));
      // Validate timestamps - ensure they're not in the future
      const currentTime = Math.floor(Date.now()/1000);
      // If timestamp is more than a day in the future, use current time
      if (storedValue.start > currentTime + 86400 || storedValue.end > currentTime + 86400) {
        console.warn("Invalid future timestamps detected in selectedMonth, resetting to current month");
        return {
          start: dayjs().startOf('month').unix(),
          end: dayjs().endOf('month').unix()
        };
...
export const selectedPeriodRange = typeof localStorage !== 'undefined' ? 
  ref(localStorage.getItem('selectedPeriodRange') ? 
    (() => {
      // Parse stored value
      const storedValue = JSON.parse(localStorage.getItem('selectedPeriodRange'));
      // Validate timestamps - ensure they're not in the future
      const currentTime = Math.floor(Date.now()/1000);
      // If timestamp is more than a day in the future, use current time
      if (storedValue.end > currentTime + 86400) {
        console.warn("Invalid future timestamp detected in selectedPeriodRange, resetting end to current time");
        storedValue.end = currentTime;
      }
      return storedValue;
    })() 
    : {value: 'allTime', label: 'All Time', start: 0, end: Math.floor(Date.now()/1000)}
  ) 
  : ref({value: 'allTime', label: 'All Time', start: 0, end: Math.floor(Date.now()/1000)})
export const selectedDashTab = typeof localStorage !== 'undefined' ? ref(localStorage.getItem('selectedDashTab') || 'charts') : ref('charts')

export const amountCase = typeof localStorage !== 'undefined' ? ref(localStorage.getItem('selectedGrossNet') || 'gross') : ref('gross')
export const amountCapital = ref()

export const dailyPageMounted = ref(false)