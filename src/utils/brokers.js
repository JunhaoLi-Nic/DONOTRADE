...
export async function useBrokerInteractiveBrokers(param, param2) {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            const sourceTimezone = window.localStorage.getItem('sourceTimezone') || 'America/New_York';
            const targetTimezone = window.localStorage.getItem('targetTimezone') || timeZoneTrade.value;
            
            console.log(`IB Import: Source timezone: ${sourceTimezone}, Target timezone: ${targetTimezone}`);

            // Sort by date/time
            papaParse.data.sort((a, b) => {
                const aDate = dayjs.tz(a["Date/Time"], "YYYYMMDD;HHmmss", sourceTimezone);
                const bDate = dayjs.tz(b["Date/Time"], "YYYYMMDD;HHmmss", sourceTimezone);
            ...
            papaParse.data.forEach(element => {
                if (element.ClientAccountID && element.AssetClass != "CASH") {
                    ...
                    // Parse the full date/time in source timezone
                    const originalDateTime = dayjs.tz(`${tempDate};${tempTime}`, "YYYYMMDD;HHmmss", sourceTimezone);
                    
                    // Convert to target timezone
                    const convertedDateTime = originalDateTime.tz(targetTimezone);
                    
                    // Format date in MM/DD/YYYY format for T/D and S/D
                    const newDate = convertedDateTime.format("MM/DD/YYYY");
                    temp["T/D"] = newDate;
                    temp["S/D"] = newDate;
                    
                    // Format time in HH:MM:SS format for Exec Time
                    const newTime = convertedDateTime.format("HH:mm:ss");
                    temp["Exec Time"] = newTime;
                    
                    // Store original and converted time for debugging
                    console.log(`Converting: ${tempDate};${tempTime} (${sourceTimezone}) → ${convertedDateTime.format("YYYYMMDD;HHmmss")} (${targetTimezone})`);
                    ...
                    // Parse commission as a number to ensure proper calculation
                    const commNum = Number(element.Commission);
                    ...
                    // Parse proceeds as a number to ensure proper calculation
                    const proceeds = parseFloat(element.Proceeds);
                    // Calculate proper proceeds based on side and price
                    const qty = parseFloat(temp.Qty);
                    const price = parseFloat(temp.Price);
                    
                    // For buy/sell calculation
                    let calculatedProceeds;
                    if (temp.Side === 'B' || temp.Side === 'BC') {
                        // Buy orders are negative proceeds (money going out)
                        calculatedProceeds = -(qty * price);
                    } else if (temp.Side === 'S' || temp.Side === 'SS') {
                        // Sell orders are positive proceeds (money coming in)
                        calculatedProceeds = qty * price;
                    } else {
                        // Fallback to provided proceeds
                        calculatedProceeds = proceeds;
                    }
                    
                    // Always use our calculated proceeds rather than the sometimes incorrect values from the broker
                    temp["Gross Proceeds"] = calculatedProceeds.toString();
                    temp["Net Proceeds"] = (calculatedProceeds + (-commNum)).toString(); // Add commission (which is negative)
                    
                    // Debug logging for proceeds calculation
                    console.log(`${temp.Symbol} ${temp.Side} ${qty}x${price}: Calculated=${calculatedProceeds}, Commission=${commNum}, Net=${calculatedProceeds + commNum}`)
                    temp["Clr Broker"] = ""
                    temp.Liq = ""
                    temp.Note = ""
                    //console.log("temp "+JSON.stringify(temp))
