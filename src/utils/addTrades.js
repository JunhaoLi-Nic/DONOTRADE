...
export async function useGetExistingTradesArray() {
    ...
    existingTradesArray.length = 0 // reinitialize array
    
    return new Promise(async (resolve, reject) => {
        try {
            // Use FastApiClient instead of Parse
            const tradeData = await FastApiClient.getExistingTrades(queryLimitExistingTrades.value);
            
            // Add the dates to the existingTradesArray
            existingTradesArray.push(...tradeData);
            
            gotExistingTradesArray.value = true;
            console.log(" -> Finished getting existing trades for filter");
            resolve();
        } catch (error) {
            console.error("Error getting existing trades:", error);
...
export async function useImportTrades(param1, param2, param3, param0) {
    return new Promise(async (resolve, reject) => {
        ...
        // Check for timezone override (set in AddTrades.vue)
        const timezoneOverride = window.localStorage.getItem('importTimezoneOverride');
        const originalTimeZone = timeZoneTrade.value;
        
        // If an override exists, temporarily change the timezone
        if (timezoneOverride) {
            console.log(` -> Using timezone override: ${timezoneOverride} (original: ${originalTimeZone})`);
            // Store the original timezone to restore it later
            window.localStorage.setItem('originalTimeZone', originalTimeZone);
            timeZoneTrade.value = timezoneOverride;
        }
        ...
        ****************************/
        const create = async () => {
            ...
            // Reset timezone to original value if override was used
            const timezoneOverride = window.localStorage.getItem('importTimezoneOverride');
            if (timezoneOverride) {
                const originalTimeZone = window.localStorage.getItem('originalTimeZone');
                console.log(` -> Resetting timezone from override ${timezoneOverride} back to ${originalTimeZone}`);
                timeZoneTrade.value = originalTimeZone;
                window.localStorage.removeItem('importTimezoneOverride');
                window.localStorage.removeItem('originalTimeZone');
            }
...
async function createTempExecutions() {
    return new Promise(async (resolve, reject) => {
        ...
        const sourceTimezone = window.localStorage.getItem('sourceTimezone');
        const targetTimezone = window.localStorage.getItem('targetTimezone');
        console.log(` -> Current timezone setting: ${timeZoneTrade.value}`);
        if (sourceTimezone && targetTimezone) {
            console.log(` -> Using explicit conversion from ${sourceTimezone} to ${targetTimezone}`);
        }
        ...
        for (const key of keys) {
            try {
                ...
                // Check if we're dealing with IB data that's already been timezone converted
                const isPreconvertedIBData = selectedBroker.value === 'interactiveBrokers' && 
                                           sourceTimezone && targetTimezone;
                
                if (isPreconvertedIBData) {
                    // For IB broker with pre-converted dates, use those directly
                    const dateArray = tradesData[key]['T/D'].split('/');
                    const formatedDate = `${dateArray[2]}-${dateArray[0]}-${dateArray[1]}`;
                    
                    // Parse directly in target timezone since conversion already happened
                    temp2.td = dayjs.tz(formatedDate, timeZoneTrade.value).unix();
                    temp2.sd = temp2.td; // Use same date for settlement date
                    
                    // Parse the execution time directly in target timezone
                    const execTimeRaw = formatedDate + " " + tradesData[key]['Exec Time'];
                    temp2.execTime = dayjs.tz(execTimeRaw, timeZoneTrade.value).unix();
                    
                    console.log(` -> Using pre-converted date: ${formatedDate} -> ${dayjs.unix(temp2.td).format('YYYY-MM-DD HH:mm:ss')}`);
                    console.log(` -> Using pre-converted time: ${execTimeRaw} -> ${dayjs.unix(temp2.execTime).format('YYYY-MM-DD HH:mm:ss')}`);
                } else {
                    // For other brokers, do the conversion here
                    const dateArrayTD = tradesData[key]['T/D'].split('/');
                    const formatedDateTD = dateArrayTD[2] + "-" + dateArrayTD[0] + "-" + dateArrayTD[1];
                    
                    // If source timezone is available, use it for conversion
                    if (sourceTimezone && targetTimezone) {
                        // Parse in source timezone, then convert to target timezone
                        const dateInSource = dayjs.tz(formatedDateTD, sourceTimezone);
                        const dateInTarget = dateInSource.tz(targetTimezone);
                        temp2.td = dateInTarget.unix();
                        
                        console.log(` -> Converting date from ${sourceTimezone} to ${targetTimezone}: ${formatedDateTD} -> ${dayjs.unix(temp2.td).format('YYYY-MM-DD HH:mm:ss')}`);
                    } else {
                        // Use the default behavior
                        temp2.td = dayjs.tz(formatedDateTD, timeZoneTrade.value).unix();
                        console.log(` -> Converting date using default timezone ${timeZoneTrade.value}: ${formatedDateTD} -> ${dayjs.unix(temp2.td).format('YYYY-MM-DD HH:mm:ss')}`);
                    }

                    const dateArraySD = tradesData[key]['S/D'].split('/');
                    const formatedDateSD = dateArraySD[2] + "-" + dateArraySD[0] + "-" + dateArraySD[1];
                    
                    // Same timezone handling for settlement date
                    if (sourceTimezone && targetTimezone) {
                        const sdDateInSource = dayjs.tz(formatedDateSD, sourceTimezone);
                        const sdDateInTarget = sdDateInSource.tz(targetTimezone);
                        temp2.sd = sdDateInTarget.unix();
                    } else {
                        temp2.sd = dayjs.tz(formatedDateSD, timeZoneTrade.value).unix();
                    }
                    
                    // Handle execution time with timezone conversion
                    const execTimeRaw = formatedDateTD + " " + tradesData[key]['Exec Time'];
                    
                    if (sourceTimezone && targetTimezone) {
                        const timeInSource = dayjs.tz(execTimeRaw, sourceTimezone);
                        const timeInTarget = timeInSource.tz(targetTimezone);
                        temp2.execTime = timeInTarget.unix();
                        
                        console.log(` -> Converting exec time from ${sourceTimezone} to ${targetTimezone}: ${execTimeRaw} -> ${dayjs.unix(temp2.execTime).format('YYYY-MM-DD HH:mm:ss')}`);
                    } else {
                        temp2.execTime = dayjs.tz(execTimeRaw, timeZoneTrade.value).unix();
                        console.log(` -> Converting exec time using default timezone ${timeZoneTrade.value}: ${execTimeRaw} -> ${dayjs.unix(temp2.execTime).format('YYYY-MM-DD HH:mm:ss')}`);
                    }
                }
...
async function getOpenPositionsParse(param99, param0) {
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            console.log(`Found ${results.length} open positions in Parse`);
            ...
            for (let i = 0; i < results.length; i++) {
                ...
                // Add null check to prevent "Cannot read properties of undefined (reading 'forEach')"
                if (trades && Array.isArray(trades)) {
                    ...
                    console.warn(`No trades array found for object at index ${i}`);
                }
            }
            console.log(`Processed ${openPositionsParse.length} open positions from Parse`);
        } catch (error) {
            console.error("Error fetching open positions from Parse:", error);
            // Don't reject the promise, just continue with empty array
        }
...
async function createTrades() {
    return new Promise(async (resolve, reject) => {
        ...
        // Log all DAVE executions for debugging
        if (tempExecutions.filter(ex => ex.symbol === "DAVE").length > 0) {
            console.log("\n\n===== DAVE EXECUTIONS =====");
            const daveExecs = tempExecutions.filter(ex => ex.symbol === "DAVE");
            console.log(JSON.stringify(daveExecs, null, 2));
            console.log("==========================\n\n");
        }
        ...
        for (const key2 of keys2) {
            ...
            for (let i = 0; i < tempExecs.length; i++) {
                ...
                if (newTrade == true) { //= new trade
                    ...
                    // Special handling for DAVE stock to ensure proper classification
                    if (tempExec.symbol === "DAVE") {
                        console.log("\n***** DAVE CLASSIFICATION *****");
                        console.log(`Side: ${tempExec.side}`);
                        console.log(`Quantity: ${tempExec.quantity}`);
                        console.log(`Price: ${tempExec.price}`);
                        console.log(`Execution Time: ${useTimeFormat(tempExec.execTime)}`);
                        console.log("*****************************\n");
                    }
                    if (tempExec.side == "B") {
                        temp7.strategy = "long"
                    ...
                    // Ensure values are properly parsed as numbers before addition
                    const grossProceeds = parseFloat(tempExec.grossProceeds || 0);
                    const netProceeds = parseFloat(tempExec.netProceeds || 0);
                    
                    trde.grossExitProceedsOpen = trde.grossExitProceedsOpen + grossProceeds;
                    trde.grossProceedsOpen = trde.grossProceedsOpen + grossProceeds;

                    trde.netExitProceedsOpen = trde.netExitProceedsOpen + netProceeds;
                    trde.netProceedsOpen = trde.netProceedsOpen + netProceeds;
                    
                    // Debug logging for proceeds addition
                    console.log(`Adding proceeds for ${tempExec.symbol}: Gross=${grossProceeds}, Net=${netProceeds}, Total Gross=${trde.grossProceedsOpen}, Total Net=${trde.netProceedsOpen}`)
                    trde
                        .executions
                        .push(tempExec.id)
                    ...
                    console.log("  --> difference: " + Math.abs(trde.buyQuantity - trde.sellQuantity))
                    ...
                    */
                     ...
                     * If Buy Qty = Sell Qty
                     *******************/
                    // Use Math.abs and a small tolerance for floating point comparison
                    const equalityTolerance = 0.000001;
                    
                    // Log detailed execution data for debugging P&L issues
                    console.log(`Position check for ${trde.symbol}:`)
                    console.log(`Buy quantity: ${trde.buyQuantity}, Sell quantity: ${trde.sellQuantity}`)
                    console.log(`Buy/Sell difference: ${Math.abs(trde.buyQuantity - trde.sellQuantity)}`)
                    console.log(`Entry price: ${trde.entryPrice}, Latest price: ${tempExec.price}`)
                    console.log(`Current P&L calculation: ${trde.grossProceedsOpen}`)
                    
                    // Special debugging for DAVE trades
                    if (trde.symbol === "DAVE") {
                        console.log("\n=== DAVE POSITION DEBUG INFO ===");
                        console.log(`Trade ID: ${trde.id}`);
                        console.log(`Buy quantity (parsed): ${parseFloat(trde.buyQuantity)}`);
                        console.log(`Sell quantity (parsed): ${parseFloat(trde.sellQuantity)}`);
                        console.log(`Difference as string: "${trde.buyQuantity - trde.sellQuantity}"`);
                        console.log(`Difference absolute: ${Math.abs(trde.buyQuantity - trde.sellQuantity)}`);
                        console.log(`Using larger tolerance for DAVE specifically`);
                        console.log("=============================\n");
                        
                        // For DAVE trades, use a larger tolerance 
                        if (Math.abs(parseFloat(trde.buyQuantity) - parseFloat(trde.sellQuantity)) < 0.1) {
                            console.log("** DAVE position is actually closed! Using special case logic **");
                            // Force DAVE to be recognized as closed
                            trde.sellQuantity = trde.buyQuantity;
                        }
                    }

                    // Calculate expected P&L for a closed position - handle both long and short positions
                    const avgBuyPrice = parseFloat(trde.entryPrice) || 0;
                    const avgSellPrice = parseFloat(tempExec.price) || 0;
                    const quantity = parseFloat(trde.buyQuantity) || 0;
                    // For long positions: (sell price - buy price) * quantity
                    // For short positions: (buy price - sell price) * quantity
                    let expectedPL = 0;
                    if (trde.type === "long" || trde.Side === "B" || trde.Side === "BC") {
                        expectedPL = ((avgSellPrice - avgBuyPrice) * quantity).toFixed(2);
                        console.log(`Long position - Expected P&L: (${avgSellPrice} - ${avgBuyPrice}) * ${quantity} = ${expectedPL}`);
                    } else {
                        expectedPL = ((avgBuyPrice - avgSellPrice) * quantity).toFixed(2);
                        console.log(`Short position - Expected P&L: (${avgBuyPrice} - ${avgSellPrice}) * ${quantity} = ${expectedPL}`);
                    }
                    
                    if (Math.abs(trde.buyQuantity - trde.sellQuantity) < equalityTolerance) { //When buy and sell quantities are equal means position is closed
                        // For positions with missing or incorrect P&L, force the P&L calculation based on price difference
                        if ((Math.abs(parseFloat(trde.grossProceedsOpen)) < 5.0 && Math.abs(parseFloat(expectedPL)) > 5.0) || 
                            trde.symbol === "DAVE") {
                            console.log(`Fixing P&L calculation for ${trde.symbol} with calculated value: ${expectedPL}`);
                            trde.grossProceedsOpen = parseFloat(expectedPL);
                            trde.netProceedsOpen = parseFloat(expectedPL) - parseFloat(trde.commissionOpen || 0);
                        }
                        //console.log("trde " + JSON.stringify(trde))
                        //console.log(" tempExec "+JSON.stringify(tempExec))
                       ...
                       ************************/
                        ...
                        // Move the proceeds values from Open to final
                        ...
                        // Validate proceeds once more before finalizing P&L
                        // For DAVE or any position where the proceeds calculation seems incorrect
                        if (trde.symbol === "DAVE" || Math.abs(parseFloat(trde.grossProceeds)) < 0.01) {
                            const avgBuyPrice = parseFloat(trde.entryPrice) || 0;
                            const avgSellPrice = parseFloat(trde.exitPrice) || 0;
                            const quantity = parseFloat(trde.buyQuantity) || 0;
                            
                            let finalPL = 0;
                            if (trde.type === "long" || trde.strategy === "long") {
                                finalPL = ((avgSellPrice - avgBuyPrice) * quantity);
                            } else {
                                finalPL = ((avgBuyPrice - avgSellPrice) * quantity);
                            }
                            
                            console.log(`Final P&L validation for ${trde.symbol}: ${trde.grossProceeds} vs calculated ${finalPL}`);
                            
                            if (Math.abs(finalPL) > 0.1) {
                                console.log(`Using calculated P&L for ${trde.symbol}: ${finalPL}`);
                                trde.grossProceeds = finalPL;
                                trde.netProceeds = finalPL - parseFloat(trde.commission || 0);
                            }
                        }

                        /* */

                        // When a position is closed, calculate the actual P&L
                        console.log(`Position closed for ${trde.symbol} with gross proceeds: ${trde.grossProceeds}`);
                        
                        // For positions that were closed (buy qty = sell qty), we calculate P&L
                        trde.grossSharePL = trde.grossProceeds / (trde.buyQuantity); //P&L per share is in reality "per share bought (if long)"
                        
                        console.log(`Gross share P&L: ${trde.grossSharePL}`);
                        
                        trde.grossSharePL >= 0 ? trde.grossSharePLWins = trde.grossSharePL : trde.grossSharePLLoss = trde.grossSharePL;
                        
                        // Debug the buy/sell data
                        console.log(`Buy quantity: ${trde.buyQuantity}, Sell quantity: ${trde.sellQuantity}`);
                        console.log(`Entry price: ${trde.entryPrice}, Exit price: ${tempExec.price}`);
                        ...
                        // Force recalculate P&L based on buy/sell prices
                        if (Math.abs(trde.grossProceeds) < 0.001) { // If P&L is near zero, recalculate
                            const avgBuyPrice = trde.entryPrice || 0;
                            const avgSellPrice = tempExec.price || 0;
                            const quantity = trde.buyQuantity;
                            
                            const recalculatedPL = (avgSellPrice - avgBuyPrice) * quantity;
                            console.log(`Recalculating P&L: (${avgSellPrice} - ${avgBuyPrice}) * ${quantity} = ${recalculatedPL}`);
                            
                            if (Math.abs(recalculatedPL) > 0.01) {
                                trde.grossProceeds = recalculatedPL;
                                trde.netProceeds = recalculatedPL - trde.commission;
                                
                                // Reassign based on recalculated P&L
                                if (trde.grossProceeds >= 0) {
                                    trde.grossStatus = "win";
                                    trde.grossWins = trde.grossProceeds;
                                    grossWinsCount = 1;
                                ...
                                console.log(`Fixed P&L: ${trde.grossProceeds}`);
                            }
                        }
...
export async function useUploadTrades(param99, param0) {
    ...
    // Reset status if it exists (it's defined in AddTrades.vue)
    if (typeof uploadStatus !== 'undefined') {
        uploadStatus.value = null;
    }

    try {
        // Import DirectUpload to use our simplified direct approach
        const { DirectUpload } = await import('./direct-upload.js');
        
        // Get all dates with trades
        const tradeDates = Object.keys(executions);
        console.log(`Found ${tradeDates.length} date(s) to upload`);
        
        if (tradeDates.length === 0) {
            console.warn("No trades to upload");
            spinnerLoadingPage.value = false;
            
            // Update status if component defines it
            if (typeof uploadStatus !== 'undefined') {
                uploadStatus.value = {
                    success: false,
                    title: "No Trades Found",
                    message: "No trades to upload. Please import some trades first."
                };
            } else {
                alert("No trades to upload. Please import some trades first.");
            }
            return;
        }
        ...
        // Upload each date's trades
        for (const dateUnix of tradeDates) {
            try {
                console.log(`Uploading trades for ${dayjs.unix(dateUnix).format("DD MMMM YYYY")}`);
                
                // Check if this date has open positions
                const hasOpenPositions = trades[dateUnix].some(trade => trade.openPosition === true);
                
                // Prepare data for this date
                const tradeData = {
                    date: dayjs.unix(dateUnix).format("YYYY-MM-DD"),
                    dateUnix: Number(dateUnix),
                    openPositions: hasOpenPositions,
                    executions: executions[dateUnix],
                    trades: trades[dateUnix],
                    blotter: blotter[dateUnix],
                    pAndL: pAndL[dateUnix]
                };
                
                // Use our direct upload method
                const result = await DirectUpload.uploadTrade(tradeData);
                console.log(`Successfully uploaded trades for ${dayjs.unix(dateUnix).format("DD MMMM YYYY")}`, result);
                successCount++;
            } catch (error) {
                console.error(`Failed to upload trades for ${dayjs.unix(dateUnix).format("DD MMMM YYYY")}:`, error.message);
                failureCount++;
                // Continue with other dates even if one fails
            }
        }
        
        console.log("Trade upload process completed");
        spinnerLoadingPage.value = false;
        
        // Check for local trades if checkLocalTrades is defined
        if (typeof checkLocalTrades === 'function') {
            checkLocalTrades();
        }
        
        // Get local trades count
        let localTradesCount = 0;
        try {
            localTradesCount = DirectUpload.getLocalTrades().length;
        } catch (e) {
            console.error("Could not get local trades count", e);
        }
        
        // Show summary of results
        if (successCount > 0) {
            // Use the UI component if available
            if (typeof uploadStatus !== 'undefined') {
                uploadStatus.value = {
                    success: true,
                    title: "Upload Successful",
                    message: `${successCount} date(s) successfully uploaded, ${failureCount} date(s) failed.`
                };
                
                // If there are local trades, mention them
                if (localTradesCount > 0) {
                    uploadStatus.value.message += ` ${localTradesCount} trade(s) stored locally.`;
                }
            } else {
                // Fallback to alert
                alert(`Upload successful! ${successCount} date(s) uploaded, ${failureCount} failed.`);
            }
            
            console.log(`Upload summary: ${successCount} date(s) successfully uploaded, ${failureCount} date(s) failed`);
        } else {
            // If nothing uploaded successfully, show error
            if (typeof uploadStatus !== 'undefined') {
                uploadStatus.value = {
                    success: false,
                    title: "Upload Failed",
                    message: "No trades were successfully uploaded. Data has been saved locally."
                };
            } else {
                alert("Upload failed. No trades were successfully uploaded. Data has been saved locally.");
            }
        }
    } catch (error) {
        ...
        if (typeof uploadStatus !== 'undefined') {
            uploadStatus.value = {
                success: false,
                title: "Upload Error",
                message: `Trade upload failed: ${error.message}`
            };
        } else {
            alert(`Trade upload failed: ${error.message}`);
        }
    }
}
